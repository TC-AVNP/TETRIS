...
(defun procura-pp (p)
	(let
		...
		(accoessolucao '()))
		; (if 
			; (equalp 1 (problema-estado-inicial p))
			; (return-from procura-pp '(RIGHT LEFT RIGHT)))
		
(if	
	(not(funcall (problema-accoes p) (problema-estado-inicial p)))
		(return-from procura-pp nil))
(if (funcall (problema-solucao p) (problema-estado-inicial p)) 
	(return-from procura-pp nil))
		(loop do
			(if 
					...
					(block bloqueio
						(if 
							...
							(progn
								...
								(setf (problema-estado-inicial problematico) estado)
						...
						(if
							(eq (funcall (problema-accoes (node-estado noded)) estado) nil)									;estado final sem solucao
							(progn														
								(setf (node-accao noded) (reverse (cdr (reverse (node-accao noded)))))
...
(defun procura-A* (p h)
	(let
		...
		; (format t "~% O PRINT DO FUNCALL->> ~a~%" (funcall fsolucao (node-A*-estado noded)))					
		(if 
...
;EXPANDIR NO
				(dolist (x (reverse (funcall faccoes (node-A*-estado noded))))   					;lista de accoes
						...
						(setf novonode (cria-node-A* 
													...
													; (format t "~%COMEA:a accao foi ~a~%" x)
						(if
							(funcall fsolucao novoestado)
							(progn
								; (print "solucao") 
								(push novonode sol)
								(push novonode opened)
								; (print (node-A*-custo novonode))
								(setf flag t)))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;BEST 'O 'E 'TAO BOM MEU DEUS 'E MESMO A  BEST;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;	
(defun heuristica-holes (tabuleiro)									;nr de buracos
	(let
		((holes 0))
		
		(dotimes (x 10)
			(setf holes (+ holes (linha-holes x tabuleiro))))
		; (format t "holes: ~a~%" holes) 
		holes))
		
(defun linha-holes (coluna tabuleiro)
	(let
		((holes 0) (flag 0))
		
		(loop for x from 17 downto 0 do 
				; (print (tabuleiro-preenchido-p tabuleiro linha x))
			(if 
				(tabuleiro-preenchido-p tabuleiro x coluna)
				(setf flag 1))
				
			(if 
				(and (eq flag 1) (not (tabuleiro-preenchido-p tabuleiro x coluna)))
				(incf holes)))
	holes))	
...
(defun heuristica-linhas (tabuleiro)  					;linhas completas
	(let
		((linhas 0))
		
		(dotimes (linha 18)
			(if
				(tabuleiro-linha-completa-p tabuleiro linha)
				(incf linhas)))
				
		; (format t "as linhas: ~a~%" linhas)
		linhas))
		
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;alutra-somada-estado: estado -> inteiro
;;devolve a soma da altura de cada coluna no tabuleiro de um estado
(defun heuristica-altura (tab)
	(let(
		(altura-somada 0))
		
	(dotimes (j 10)
		(setf altura-somada (+ altura-somada (tabuleiro-altura-coluna tab j))))
		
		; (format t "o altura-somada: ~a~%" altura-somada)
	altura-somada))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;	
;;;desfasamento-colunas: estado -> inteiro
;;Da a variacao entre as alturas das colunas do tabuleiro de um estado. 
;;soma-se a diferenca de altura entre todas as colunas adjacentes (valores absolutos)	
(defun heuristica-desfasamento (tab)
	(let(
		(desfasamento 0)
		(iterador-pares 0))
	(dotimes (j 9) ; -1 pq queremos ir apenas ate a penultima coluna e comparar com a col. a sua direita
		(setf iterador-pares (abs (- (tabuleiro-altura-coluna tab j) (tabuleiro-altura-coluna tab (+ j 1)))))
		(setf desfasamento (+ desfasamento iterador-pares)))
		; (format t "o desfazamento: ~a~%" desfasamento)
	desfasamento))	
		
(defun A-HEURISTICA-A-MELHOR-HEURISTICA	 (estado)
	(let
		((valorsSUPREMO) (tab (estado-tabuleiro estado))
		(CONSTANTE-A 0.6) (CONSTANTE-B -0.85) (CONSTANTE-C 0.3) (CONSTANTE-D 0.15))
		
		(setf valorsSUPREMO (+ 
								(* CONSTANTE-A (heuristica-altura tab))
								(* CONSTANTE-B (heuristica-linhas tab))
								(* CONSTANTE-C (heuristica-holes tab))
								(* CONSTANTE-D (heuristica-desfasamento tab))))
		valorsSUPREMO))
		
		
		
(defun procura-best (tab pecas)

		(let
			((problema (make-problema :estado-inicial (make-estado :pontos 0 :pecas-por-colocar pecas :pecas-colocadas '() :tabuleiro tab) 
			:solucao #'solucao :accoes #'accoes :resultado #'resultado :custo-caminho #'custo-BEST)))
			
			(procura-A* problema #'A-HEURISTICA-A-MELHOR-HEURISTICA)))
...
; (compile-file "tetris.lisp")
	...
	; (defun pontos (e a)
		; (let ((estado e))
			; (dolist (x a)
				; (setf estado (resultado estado x)))
...
; (setf *d* 1)
; (defun smart(e a1)
	; (let ((maior 0) (af 0) (bf 0) (cf 0) (df 0)(kek 0))
	; (loop for a from -20 upto 20 do
		; (setf *a* a)
		; (loop for b from -20 upto 5 do
			; (setf *b* b)
				; (loop for c from -20 upto 20 do
					; (setf *c* c)
					; (loop for d from -20 upto 20 do
						; (setf *d* d)
						; (setf kek (pontos e a1))
						; (if
							; (> kek maior)
							; (progn
								; (setf maior kek)
		...
		; (defun smart(e a1)
	; (let ((maior 0) (af 0) (bf 0) (cf 0) (df 0)(kek 0))
	; (loop for a from upto 1 do
		; (setf *a* a)
		; (loop for b from -1 upto 1 do
			; (setf *b* b)
				; (loop for c from -1 upto 1 do
					; (setf *c* c)
					; (loop for d from -1 upto 1 do
						; (setf *d* d)
						; (setf kek (pontos e a1))
						; (if
							; (> kek maior)
...
;;;deve retornar IGNORE
; (setf r1 (procura-best '#2A((T T T T NIL T T T T T)(T T T NIL NIL NIL T T T T)(T T T NIL NIL NIL T T T T)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)) '(t)))
; (setf a1 '#2A((T T T T NIL T T T T T)(T T T NIL NIL NIL T T T T)(T T T NIL NIL NIL T T T T)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)))

; (setf e1 (make-estado :tabuleiro (array->tabuleiro a1) :pecas-por-colocar '(t) :pontos 0 :pecas-colocadas '()))
; (setf prot (make-problema :estado-inicial e1 :solucao #'solucao :accoes #'accoes :resultado #'resultado :custo-caminho #'custo-oportunidade))
; (setf r1 (procura-A* prot #'A-HEURISTICA-A-MELHOR-HEURISTICA))
;;;deve retornar T
; (>= (executa-jogadas (make-estado :tabuleiro (array->tabuleiro a1) :pecas-por-colocar '(t i l l) :pontos 0 :pecas-colocadas '()) r1) 500)
; (print (procura-A* prot #'(lambda (x) 0)))
...
; (print (procura-best '#2A((T T T T NIL T T T T T)(T T T NIL NIL NIL T T T T)(T T T NIL NIL NIL T T T T)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)) '(t)))
	;;; Teste 25 E2
;;; procura-best num tabuleiro com 4 jogadadas por fazer. Os grupos tem um tempo limitado para conseguir obter pelo menos 500 pontos. 
;;; deve retornar IGNORE
; (ignore-value (setf a1 '#2A((T T T T NIL NIL T T T T)(T T T NIL NIL NIL T T T T)(T T T NIL NIL NIL T T T T)(T T T NIL NIL NIL T T T T)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL)(NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL))))
